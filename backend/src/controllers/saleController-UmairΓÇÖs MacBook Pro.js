...
exports.update = (req, res) => {
  try {
    const { id } = req.params;
    const { saleDate, gatePassNo, billNo, remarks } = req.body;
    const existing = db.prepare('SELECT * FROM sales WHERE id = ?').get(id);
    if (!existing) return res.status(404).json({ error: 'Sale not found' });
    db.prepare('UPDATE sales SET sale_date=?, gate_pass_no=?, bill_no=?, remarks=?, updated_at=CURRENT_TIMESTAMP WHERE id=?')
      .run(saleDate ?? existing.sale_date, gatePassNo ?? existing.gate_pass_no, billNo ?? existing.bill_no, remarks ?? existing.remarks, id);
