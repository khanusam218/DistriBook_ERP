...
function postCashBankEntry(accountId, txnType, refId, credit, description, date) {
...
function recalcBalances(employeeId) {
  ...
  for (const entry of entries) {
    ...
    db.prepare('UPDATE employee_ledger SET balance = ? WHERE id = ?').run(running, entry.id);
  }
}

function getLastBalance(employeeId) {
...
exports.delete = (req, res) => {
  try {
    const row = db.prepare('SELECT * FROM employees WHERE id = ?').get(req.params.id);
    if (!row) return res.status(404).json({ error: 'Employee not found' });
    db.prepare('DELETE FROM employee_ledger WHERE employee_id = ?').run(req.params.id);
...
exports.addAdvance = (req, res) => {
  try {
    ...
    const prevBal = getLastBalance(id);
    ...
    if (bankAccountId) {
      postCashBankEntry(bankAccountId, 'ADVANCE', r.lastInsertRowid, creditAmt, `Advance â€” ${employee.name}${desc !== 'Cash Advance' ? ': ' + desc : ''}`, date);
    }

    res.status(201).json(db.prepare('SELECT * FROM employee_ledger WHERE id = ?').get(r.lastInsertRowid));
  } catch (e) { res.status(500).json({ error: 'Employee operation failed' }); }
};
...
exports.addOvertime = (req, res) => {
  try {
    ...
    if (bankAccountId) {
      postCashBankEntry(bankAccountId, 'OVERTIME', r.lastInsertRowid, credit, `OT Payment â€” ${employee.name} (${otHours} hrs)`, date);
    }
...
exports.addSalary = (req, res) => {
  try {
    ...
    if (bankAccountId) {
      postCashBankEntry(bankAccountId, 'SALARY', r.lastInsertRowid, credit, `${desc} â€” ${employee.name}`, date);
    }
...
exports.addPaymentReceived = (req, res) => {
  try {
    const { id } = req.params;
    const employee = db.prepare('SELECT * FROM employees WHERE id = ?').get(id);
    if (!employee) return res.status(404).json({ error: 'Employee not found' });
    ...
    if (bankAccountId) {
      ...
      const row = db.prepare('SELECT COALESCE(SUM(debit - credit), 0) as txn FROM cash_bank_transactions WHERE account_id = ?').get(bankAccountId);
...
exports.deleteEntry = (req, res) => {
  try {
    ...
    const entry = db.prepare('SELECT * FROM employee_ledger WHERE id = ? AND employee_id = ?').get(entryId, id);
    if (!entry) return res.status(404).json({ error: 'Entry not found' });
    db.prepare('DELETE FROM employee_ledger WHERE id = ?').run(entryId);
    recalcBalances(id);
