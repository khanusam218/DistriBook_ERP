...
exports.getById = async (req, res) => {
  try {
    const record = await db.prepare('SELECT * FROM purchase_returns WHERE id = ?').get(req.params.id);
  ...
  try {
    ...
    const run = db.transaction(async () => {
      ...
      for (const item of items) {
        ...
        const alreadyReturned = alreadyReturnedRow.qty || 0;
      ...
      for (const item of items) {
        ...
        const stockInfo = await db.prepare('SELECT pieces_per_ctn FROM stocks WHERE id = ?').get(item.stockId);
      ...
      await db.prepare('UPDATE purchase_returns SET total_amount = ? WHERE id = ?').run(totalAmount, returnId);

      const purchase = await db.prepare('SELECT * FROM purchases WHERE id = ?').get(purchaseId);
      const vendorId = purchase.vendor_id;
      const prevBalanceRow = await db.prepare(
...
exports.delete = async (req, res) => {
  try {
    const run = db.transaction(async () => {
      const { id } = req.params;
      const items = await db.prepare('SELECT stock_id, quantity FROM purchase_return_items WHERE purchase_return_id = ?').all(id);
      for (const item of items) {
      ...
      const row = await db.prepare('SELECT * FROM purchase_returns WHERE id = ?').get(id);
      if (!row) return null;
      await db.prepare('DELETE FROM vendor_ledger WHERE reference_id = ? AND reference_type = ?').run(id, 'purchase_return');
