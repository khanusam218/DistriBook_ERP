...
exports.getAll = async (req, res) => {
  try {
    res.json(await db.prepare('SELECT * FROM company_ledger ORDER BY transaction_date DESC').all());
  } catch (error) {
...
exports.getByAccount = async (req, res) => {
  try {
    ...
    const entries = await db.prepare('SELECT * FROM company_ledger WHERE account_name = ? ORDER BY transaction_date DESC').all(accountName);
...
exports.create = async (req, res) => {
  try {
    ...
    res.status(201).json(await db.prepare('SELECT * FROM company_ledger WHERE id = ?').get(result.lastInsertRowid));
  } catch (error) {
    res.status(500).json({ error: 'Company ledger operation failed' });
  }
};
