...
async function getVendorBalance(vendorId) {
  const row = await db.prepare('SELECT COALESCE(SUM(debit - credit), 0) as balance FROM vendor_ledger WHERE vendor_id = ?').get(vendorId);
  return row.balance || 0;
}

async function getCustomerBalance(customerId) {
  const row = await db.prepare('SELECT COALESCE(SUM(debit - credit), 0) as balance FROM customer_ledger WHERE customer_id = ?').get(customerId);
  return row.balance || 0;
}

async function createVendorLedgerEntry(vendorId, transactionType, referenceId, referenceType, debit, credit, description) {
  const newBalance = (await getVendorBalance(vendorId)) + debit - credit;
...
async function createCustomerLedgerEntry(customerId, transactionType, referenceId, referenceType, debit, credit, description) {
  const newBalance = (await getCustomerBalance(customerId)) + debit - credit;
  const result = await db.prepare(
...
async function recordOpeningBalance(vendorId, customerId, openingBalance, type) {
