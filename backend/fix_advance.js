  ...
  for (const file of dbFiles) {
    ...
    try {
      ...
      if (badAdvances.length > 0 && badAdvances[0].values.length > 0) {
        ...
        // Recalculate balances per employee (use string interpolation - safe, empId is numeric)
        const empIds = [...new Set(badAdvances[0].values.map(r => r[1]))];
        for (const empId of empIds) {
          const entries = db.exec(`SELECT id, debit, credit FROM employee_ledger WHERE employee_id = ${empId} ORDER BY date ASC, id ASC`);
          if (entries.length > 0) {
            let running = 0;
            ...
            console.log(`  Employee ${empId} final balance: ${running}`);
          }
        }
